...
AnalysisKey AvmusatovLoopAnalysis::Key;

AvmusatovLoopAnalysis::Result AvmusatovLoopAnalysis::run(Loop& L, LoopAnalysisManager& LAM, LoopStandardAnalysisResults& LSAR) {
    Result result;
    ...
    if(PHINode* indVar = L.getCanonicalInductionVariable()) {
        ...
        result.updatesCount = updates.size();
    }
