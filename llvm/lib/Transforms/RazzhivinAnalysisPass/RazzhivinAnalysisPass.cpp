...
#define DEBUG_TYPE "RazzhivinAnalysisPass"
...
void innLoops(Loop* loop, LoopAnalysisManager& LAM, LoopStandardAnalysisResults& LAS){
    if (loop->isInnermost()){
        auto& VLA = LAM.getResult<RazzhivinLoopAnalysis>(*loop, LAS);
        if (VLA.updates == 1)
            TotalVecLoops++;
    }
...
PreservedAnalyses RazzhivinAnalysisPass::run(Function& func, FunctionAnalysisManager& AM){
    auto& VAP = AM.getResult<RazzhivinAnalysis>(func);
    TotalAdds += VAP.adds;
    TotalMuls += VAP.muls;
