...
llvm::Value* PowExpr::gen(llvm::IRBuilder<> *builder, llvm::LLVMContext &context) const {
  ...
  auto y = builder->CreateSIToFP(v2, llvm::Type::getDoubleTy(context));
  llvm::Value* ans = builder->CreateBinaryIntrinsic(llvm::Intrinsic::pow, x, y, nullptr, "powtmp");
  return builder->CreateFPToSI(ans, llvm::Type::getInt32Ty(context));
}